...
def generate_dataset(nbr):
    face_classifier = cv2.CascadeClassifier\
        (r"C:\Users\praty\PycharmProjects\AttendanceSystem\resources\haarcascade_frontalface_default.xml")
...
def train_classifier(nbr):
    dataset_dir = r"C:\Users\praty\PycharmProjects\AttendanceSystem\dataset"
...
def fr_page():
    mycursor.execute("SELECT a.accs_id, a.accs_prsn, b.prs_name, b.prs_skill, a.accs_added, "
                     "CASE WHEN ROW_NUMBER() OVER (PARTITION BY a.accs_prsn ORDER BY a.accs_added) = 1 "
                     "     THEN 'Entry' ELSE 'Exit' END AS attendance_status "
                     "FROM accs_hist a "
...
@app.route('/loadData', methods=['GET', 'POST'])
def loadData():
    mycursor.execute("SELECT a.accs_id, a.accs_prsn, b.prs_name, b.prs_skill, "
                     "DATE_FORMAT(a.accs_added, '%H:%i:%s'), "
                     "CASE WHEN ROW_NUMBER() OVER (PARTITION BY a.accs_prsn ORDER BY a.accs_added) = 1 "
                     "     THEN 'Entry' ELSE 'Exit' END AS attendance_status "
                     "FROM accs_hist a "
                     "LEFT JOIN prs_mstr b ON a.accs_prsn = b.prs_nbr "
